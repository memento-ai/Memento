...
import { stripCommonIndent } from "./strip-common-indent";
...
export class AddPackageReadmeAgent extends Agent {
    ...
    static makePrompt() {
        return stripCommonIndent(`
            ...
            as the content of the first 'user' message of the conversation. The source files may include
            an existing README.md for the package. If so, your response will replace the existing README.md.
            Please do make improvements that seem warranted, i.e. if any sample code is out of date or
            any significant features are not documented, but retaining the existing README.md content will
            often be acceptable.
