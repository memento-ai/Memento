...
import { USER, ASSISTANT, Message, AssistantMessage } from "@memento-ai/types";
...
export class GoogleConversation implements ConversationInterface {
    ...
    async sendMessage(args: SendMessageArgs): Promise<AssistantMessage> {
        ...
        const model: GenerativeModel = this.session.client.getGenerativeModel({ model: this.model, systemInstruction: prompt});
